...
if (isset($_POST['txtFechaInicio']) && isset($_POST['txtFechaFin'])) {

    $mostrarTabla = true;
    $fechaInicio = $_POST['txtFechaInicio'];
    $fechaFin = $_POST['txtFechaFin'];
    $listaReporte = $rutaDB->reporte($fechaInicio, $fechaFin);
}
$suma = 0;
$cantidadViajes = 0;
$patente = "";

$listaPreciosTotales = 0;
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <?php
                                            $nroEntregas = 0;
                                            $listaVehiculoRuta = array();
                                            foreach ($entregaLista as $e) {
                                                if ($e->ruta_id == $r->id) {
                                                    $nroEntregas += 1;
                                                    $listaVehiculoRuta[] = $e->vehiculo_id;
                                                    $vehiculo = $vehiculoDB->buscar($e->vehiculo_id);
                                                    $precioTotalRuta += $vehiculo->precio;
                                                }
                                            }
                                            $listaFiltrada = array_unique($listaVehiculoRuta);
                                            echo '<td>';
                                            foreach ($listaFiltrada as $lf) {
                                                $vehiculo = $vehiculoDB->buscar($lf);
                                                echo $vehiculo->patente . " ";
                                            }
                                            echo '</td>';
                                            echo "<td>" . $nroEntregas . "</td>";
                                            $cantidadViajes += $nroEntregas;
                                            echo "<td>" . $precioTotalRuta . "</td>";
                                            $listaPreciosTotales += $precioTotalRuta;
                                            ?>
